...
- **CLI options**: `--output-format json|text` (`--live` requires JSON and auto-switches), `--allowed-tools "..."`, `--no-continue` (fresh session each loop) / `--continue` (force session continuity, overriding a `.ralphrc` `SESSION_CONTINUITY=false`)
